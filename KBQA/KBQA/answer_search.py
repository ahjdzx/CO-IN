...
class AnswerSearcher(object):
    def __init__(
        self, uri="bolt://172.16.20.147:7687", user="neo4j", password="secret"
    ):
    ...
    def answer_prettify(self, question_type, answer):
            ...
            for record in answer:
                for node in record:
                    desc += (
                        str(node["name"])
                        + "的口味是"
                        + str(node[self.recipe_properties_list["口味"]])
                    )
                    nodes.append({"src": node["nodeId"]})
        # 查询菜谱工艺：___的工艺是____
            ...
            for record in answer:
                for node in record:
                    desc += (
                        str(node["name"])
                        + "的工艺是"
                        + str(node[self.recipe_properties_list["工艺"]])
                    )
                    nodes.append({"src": node["nodeId"]})
        # 查询菜谱耗时：做___需要____
            ...
            for record in answer:
                for node in record:
                    desc += (
                        ...
                        + str(node["name"])
                        + "需要"
                        + str(node[self.recipe_properties_list["耗时"]])
                    )
                    nodes.append({"src": node["nodeId"]})
        # 查询食材/主食材/辅料
        # 1. ___的食材/主食材/辅料有___
        # 2. 我不知道___的食材/主食材/辅料有哪些
            ...
            or question_type == "ingredients_prime"
            or question_type == "ingredients_sub"
        ):
            ...
            for record in answer:
                ...
                nodes.append(
                    {"src": record[1]["nodeId"], "target": record[0]["nodeId"]}
                )
            if len(tmp) == 0:
