...
class DictConstructor:
    ...
    def save_material(self, project_id, path):
        ...
        cql = "MATCH (m:node) where m.projectId={0} and m.group='食材' return m".format(
            project_id
        )
    ...
    def save_cuisine(self, project_id, path):
        filename = "caixi.txt"
        cql = "MATCH (m:node) where m.projectId={0} and m.group='菜系' return m".format(
            project_id
        )
        answer = self.graph.run(cql)
        with open(path + filename, "w", encoding="utf-8") as f:
    ...
    def save_recipe(self, project_id, path):
        ...
        cql = "MATCH (m:node) where m.projectId={0} and m.group='菜谱' return m".format(
            project_id
        )
        answer = self.graph.run(cql)
